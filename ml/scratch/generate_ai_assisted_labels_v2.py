...
OUTPUT_LABELS_V2_JSON = os.path.join(OUTPUT_DIR, "ai_assisted_labels_v2.json")
OUTPUT_PRIORITY_V2_JSON = os.path.join(OUTPUT_DIR, "human_review_priority_v2.json")
OUTPUT_COMPARISON_MD = os.path.join("ml/reports", "ai_assisted_label_v1_v2_comparison.md")

def evaluate_v2_logic(row):
    ...
    if near_mine or near_quarry: infra_ev.append("mine_quarry_flag=True")
    if near_refinery or near_factory: infra_ev.append("factory_refinery_flag=True")
    
    # V2 Precedence Sequence:
    # 1. Acute/Abnormal Industrial Event (Evaluated FIRST to prevent absorption by persistent source)
    if (near_refinery or near_factory or builtup_frac >= 0.3) and max_frp >= 150.0:
        label = "industrial_fire_or_abnormal_event"
        confidence = "HIGH" if max_frp >= 250.0 else "MEDIUM"
        requires_review = (confidence == "MEDIUM")
        reasoning = f"Acute high FRP ({max_frp:.1f}MW >= 150MW) in industrial/refinery/factory/builtup context."
        
    # 2. Mining or Other Industrial Activity (Evaluated BEFORE general persistent source if mine/quarry context present)
    ...
    elif dist_fac <= 2.0 and active_days >= 10.0:
    ...
    # 4. Wildfire / Forest Fire
        ...
        confidence = "HIGH" if (forest_frac >= 0.6 and max_frp >= 50.0) else "MEDIUM"
        requires_review = (confidence == "MEDIUM")
    ...
    # 5. Agricultural Burning
        ...
        confidence = "HIGH" if (crop_frac >= 0.6 and active_days <= 2.0) else "MEDIUM"
        requires_review = (confidence == "MEDIUM")
...
def generate_v2_labels():
    ...
    v2_labels = [evaluate_v2_logic(row) for _, row in df.iterrows()]
    
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    with open(OUTPUT_LABELS_V2_JSON, "w") as f:
        json.dump(v2_labels, f, indent=2)
        
    # Generate Ranked Human Review Priority List v2
    priority_list_v2 = sorted(
        v2_labels,
        key=lambda x: (
    ...
    with open(OUTPUT_PRIORITY_V2_JSON, "w") as f:
        json.dump(priority_list_v2, f, indent=2)
        
    print(f"AI-assisted v2 labeling complete! Saved to {OUTPUT_LABELS_V2_JSON}")
